...
The transport now uses the LCD-only API from G19USB and its column-major RGB565 encoder. Open failures and write failures dispose the connection and retry after five seconds; synchronous writes report failures to the existing loop. Upstream also retries failed transfers internally. RGB565 reduces color precision from the rendered 32-bit bitmap. The watchdog still checks every 15 seconds and restarts the dashboard and Core Temp task; its LCore launch was removed to avoid competing USB owners. Keyboard input, macros, M-key LEDs, and brightness settings remain untouched. RGB requests from the bridge are now applied through the shared USB handle with exact feature report 07 colors and hardware readback.
...
## Shared lighting
Run the updated bridge and dashboard together under the same Windows user. The dashboard hosts G19Dashboard.Lighting.v1, accepting three RGB bytes and returning three hardware-readback bytes. Access is restricted to the current user; USB lifecycle, LCD writes, and lighting are serialized. Pipe failure causes the bridge to reconnect after five seconds and send its latest color. Lighting runs independently of the dashboard refresh interval. Tests cover fragmented requests, colors, and reconnection.

Live shared-USB validation: over 500 animation writes matched hardware readback while LCD updates continued. Restarting the dashboard broke the pipe as expected; the bridge reconnected after five seconds. Both updated watchdogs were restarted. Physical unplug/replug and sleep/resume remain untested.
